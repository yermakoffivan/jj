...
pub async fn find_divergent_changes(
    ...
    for (change_id, commits) in &mut result {
        sort_divergent_commits(repo, change_id, commits).await?;
    }
    Ok(result)
}
...
async fn sort_divergent_commits(
    repo: &ReadonlyRepo,
    change_id: &ChangeId,
    commits: &mut [Commit],
) -> Result<(), ConvergeError> {
    let resolved_change_targets = repo.resolve_change_id(change_id).await?;
